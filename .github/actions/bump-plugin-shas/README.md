...
| `fail-on-unpinned-autoexec` | `"false"` | if `true`, a bump candidate whose tree **at the new SHA** declares an MCP server launched via `npx`/`bunx`/`uvx`/`pipx` with a **floating** package spec (`@latest`/dist-tag, version range, or a bare unversioned name that isn't locally vendored) is **held** (per-entry skip — the pin does not advance) until upstream pins an exact version. Such a server auto-launches registry-resolved code at session start, which `source.sha` does not fix — advancing the pin would re-bless content it can't freeze. Default `false`: detect + `::warning` only, the bump proceeds. Classifier shared with the `scan-plugins` action (`../scan-plugins/lib/pin-check.sh`). |
| `launch-shape-waivers` | `""` | optional path (caller's checkout) to a package-grained waiver file for adjudicated exceptions to the pin gate — same format as the `scan-plugins` input of the same name; **every** floating spec must match a granted prefix. A SET-but-missing path fails the whole run (same posture as `tracking-config`). |
